...
[data, theta_target] = initialize_rotor_model(); 
N = data.N;
...
scale.d0 = 1e-2 * k_ref ;      % Mistuning: k_phys = d0_norm * k_ref
scale.ar = 1e-5 * k_ref ;          % Aero Real:  Re(A)_phys = ar_norm * k_ref
scale.ai = 1e-3 * k_ref ;          % Aero Imag:  Im(A)_phys = ai_norm * k_ref
...
fixed_params = [tgt_ar_norm; tgt_ai_norm; tgt_fr_norm; tgt_fi_norm];

wrapper_obj = @(t_d0) rotor_objective_wrapper(t_d0, fixed_params, data);

% costraint
Aeq = [];   
beq = [];

%% Global Minimul with Multistart
% rng default 
% num_start_points = 10; 
% 
% fprintf('Generazione di %d punti tramite Ipercubo Latino (Maximin)...\n', num_start_points);
% lhs_unit = lhsdesign(num_start_points, N, 'criterion', 'maximin', 'iterations', 50);
% 
% start_points_matrix = repmat(lb_d0', num_start_points, 1) + ...
%                       repmat((ub_d0 - lb_d0)', num_start_points, 1) .* lhs_unit;
% 
...
% fprintf('Avvio Algoritmo Genetico per esplorazione dello spazio di ricerca...\n');
% pop_size = 20 * N;
% 
% lhs_unit = lhsdesign(pop_size, N, 'criterion', 'maximin', 'iterations', 50);
% 
% lb_row = lb_d0(:)'; 
% ub_row = ub_d0(:)';
% 
% initial_pop_matrix = repmat(lb_row, pop_size, 1) + ...
%                      repmat(ub_row - lb_row, pop_size, 1) .* lhs_unit;
% 
% ga_opts = optimoptions('ga', ...
%     'PopulationSize', pop_size, ...                
%     'InitialPopulationMatrix', initial_pop_matrix, ... 
%     'MaxGenerations', N*N, ...                     
%     'CrossoverFraction', 0.5, ...                  
%     'SelectionFcn', {@selectiontournament, 2}, ... 
%     'EliteCount', ceil(0.01 * pop_size), ...           
%     'Display', 'iter', ...
%     'UseParallel', true, ...
%     'FunctionTolerance', 1e-8);
% 
% [x_ga_best, fval_ga, exitflag_ga, output_ga, population, scores] = ga(wrapper_obj, N, [], [], [], [], lb_d0, ub_d0, [], ga_opts);
% 
% figure('Name', 'Analisi Popolazione', 'Color', 'w');
% ax = axes;
% hold(ax, 'on');
% 
% pop_norm = (population - repmat(lb_d0', size(population,1), 1)) ./ ...
%            repmat((ub_d0 - lb_d0)', size(population,1), 1);
% 
% my_cmap = jet(256); 
...
% end
% 
% score_indices = floor((scores - min_score) / (max_score - min_score) * 255) + 1;
% [~, sort_idx] = sort(scores, 'descend'); 
% 
% for k = 1:length(sort_idx)
%     i = sort_idx(k); 
%     col = my_cmap(score_indices(i), :);
% 
%     line(1:N, pop_norm(i, :), 'Color', [col, 0.4], 'LineWidth', 0.8);
% end
% 
% title('Distribuzione Parametri Popolazione Finale (d_0 normalizzato)');
% xlabel('Indice Pala');
% ylabel('Valore Parametro Normalizzato (0=LB, 1=UB)');
% grid on; axis tight;
% ylim([0 1]);
% xlim([1 N]);
% xticks(1:N); 
...
% colormap(ax, my_cmap);
% c = colorbar;
% c.Label.String = 'Valore Funzione Obiettivo (Fitness)';
% clim([min_score max_score]); 
% 
% hold off;
% 
% startPoint = x_ga_best(:);
startPoint = theta0_d0;
%% solver
...
[theta_norm_opt, final_J, exitflag, output] = fmincon(wrapper_obj, ...
                                    startPoint, [], [], [], [], lb_d0, ub_d0, [], options);

%% grafici
theta_full_reconstructed = [theta_norm_opt; fixed_params];
...
%% Helper fuction
function [J_data, grad_d0] = rotor_objective_wrapper(d0_norm, fixed_params, data) 
    theta_full = [d0_norm(:); fixed_params];
    
    [J_data, grad_full] = rotor_objective_with_grad(theta_full, data);
    grad_d0 = grad_full(1:data.N);
end

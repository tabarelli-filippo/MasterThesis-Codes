...
scale.ai = 1e-2* k_ref ;          % Aero Imag:  Im(A)_phys = ai_norm * k_ref
...
theta0_ar = 1*ones(N-1, 1); 
lb_ar = -10 * ones(N-1, 1);
ub_ar =  10 * ones(N-1, 1);

%  Wrapper function for mistuning only variables
fixed_params1 = tgt_d0_norm;
ar_end = 0;
fixed_params2 = [ar_end; tgt_ai_norm; tgt_fr_norm; tgt_fi_norm];

wrapper_obj = @(t_d0) rotor_objective_wrapper(t_d0, fixed_params1, fixed_params2, data);
...
% 2. Aero Stiffness
subplot(2, 2, 2);
bar(1:N, ar_tgt, 'FaceColor', col_bar, 'EdgeColor', [0.5 0.5 0.5],'DisplayName', 'Exact Model');
hold on;
...
function [J, grad_ar] = rotor_objective_wrapper(theta, fixed_params1, fixed_params2, data)
    theta_full = [fixed_params1; theta; fixed_params2];
    
    [J, grad_full] = rotor_objective_with_grad(theta_full, data);
    N = data.N;
    grad_ar = grad_full(N+1:2*N-1);
end
